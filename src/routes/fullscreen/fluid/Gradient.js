...
export class Gradient {
    ...
    getOption(name, defaultValue = undefined) {
        if (defaultValue === undefined && name in this.defaultOptions) {
            defaultValue = this.defaultOptions[name]
        }
    ...
    animate(event = 0) {
        const shouldSkipFrame = !!window.document.hidden || !this.playing || parseInt(event, 10) % 2 === 0

        if (!shouldSkipFrame) {
            this.time += Math.min(event - this.lastFrame, 1000 / 8)
        ...
        if (this.lastFrame !== 0 && this.getOption("static")) {
            this._minigl.render()
